export default function cut () {
   ...
   * @param color  边界颜色
   */
  self.setBoundStyle = ({color = '#04b00f', mask = 'rgba(0, 0, 0, 0.56)', lineWidth = 1} = {}) => {
    const boundOption = [
//      {
...
//      },
    ...
    self.ctx.setStrokeStyle('#ffffff')
    self.ctx.setLineWidth(lineWidth)

    let y1 = (boundHeight - height) / 2 - lineWidth
    let x1 = (boundWidth - width) / 2 - lineWidth

    let y2 = boundHeight - (boundHeight - height) / 2 + lineWidth
    let x2 = boundWidth - (boundWidth - width) / 2 + lineWidth

    self.ctx.setLineDash([5, 8], 2)
    self.ctx.beginPath()
    self.ctx.moveTo(x1, y1)
    self.ctx.lineTo(x2, y1)
    self.ctx.lineTo(x2, y2)
    self.ctx.lineTo(x1, y2)
    self.ctx.lineTo(x1, y1)
    self.ctx.stroke()

//    self.ctx.fillRect(0, 0, 150, (boundHeight - height) / 2)
